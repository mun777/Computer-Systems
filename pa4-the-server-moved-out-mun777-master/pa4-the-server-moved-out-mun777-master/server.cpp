...
int main (int argc, char* argv[]) {
	...
	TCPRequestChannel* control_channel = new TCPRequestChannel("",port_num);
	while (true) {
		int clientfd = control_channel->accept_conn();
		TCPRequestChannel* newpipe =  new TCPRequestChannel(clientfd);
		thread clientthread(handle_process_loop,newpipe);
		clientthread.detach();
	}
