...
// change all insrances of fifo request channel to tcprequest channle
// change to oapporpiate consturctor
//getopt to add a and r
struct histogram
{
    int person;
    double ECG;
...
void file_thread_function (string filename, BoundedBuffer* requestbuf, TCPRequestChannel* pipe, int buffsize) {
    ...
    pipe->cwrite ((char *) &q, sizeof (MESSAGE_TYPE));
    delete pipe;

}

void worker_thread_function (BoundedBuffer*request, BoundedBuffer*response, TCPRequestChannel* pipe, int buffsize) {
...
int main (int argc, char* argv[]) {
    ...
    vector<TCPRequestChannel*> worker_chan;
    ...
    for(int i=0; i <w; i++)
    {
        TCPRequestChannel *new_chan = new TCPRequestChannel(ip_add,port_num);
    ...
    if(f!="")
    {
        TCPRequestChannel *filepipe = new TCPRequestChannel(ip_add,port_num);
        file_thread = thread(file_thread_function,f,&request_buffer,filepipe,m);
